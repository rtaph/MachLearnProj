...
library(knitr); library(plyr); library(tree); library(caret)
...
A review of the testing dataset reveals that there are only 59 predictor variables. This number is far lower than the training set, which has `r 154-60` extraneous variables. To train the model, we select the feature space that exists both in the training and testing set:
...
```{r chunkBoost, cache=TRUE}
  # Boosted tree model
  set.seed(981)
  fit.gbm <- train(classe~., data = training, method = "gbm", verbose = FALSE)
  fit.gbm
```
